...
class ContentGenerationService:
    ...
    # ===== MỚI: Chuyển outline thành text với config =====
    def _format_outline_with_config(self, outline_items: List[OutlineItem]) -> str:
        """
        Chuyển outline structured thành text format cho AI
        Bao gồm cả config của từng heading nếu có
        """
        if not outline_items:
            return "Không có outline"
        
        formatted_lines = []
        for item in sorted(outline_items, key=lambda x: x.order):
            indent = "  " * (item.level - 1)
            heading_prefix = "#" * item.level
            
            line = f"{indent}{heading_prefix} {item.title}"
            
            # Nếu heading này có config, thêm thông tin config
            if item.config:
                config_parts = []
                if item.config.word_count:
                    config_parts.append(f"word_count={item.config.word_count}")
                if item.config.keywords:
                    config_parts.append(f"keywords={', '.join(item.config.keywords)}")
                if item.config.tone:
                    config_parts.append(f"tone={item.config.tone}")
                if item.config.internal_link:
                    config_parts.append(f"internal_link={item.config.internal_link}")
    ...
    def _generate_system_prompt(self, config: ContentConfig, 
        ...
        # ===== THAY ĐỔI: Format outline với config =====
        outline_text = self._format_outline_with_config(outline) if outline else "Không có outline"
        ...
        prompt = template.format(
            ...
            article_title=title or "Chưa có tiêu đề",
    ...
    async def generate_seo_content(self, request: ContentRequest) -> ContentResponse:
        ...
        try:
            ...
            # 2. Generate system prompt (giờ nhận List[OutlineItem])
            system_prompt = self._generate_system_prompt(
                config=config,
                title=request.title,  
                outline=request.outline,  # List[OutlineItem] thay vì str
            ...
            if request.top_news:
                ...
                # ===== Format outline với config =====
                outline_text = self._format_outline_with_config(request.outline) if request.outline else "Không có outline"
                
                question_input = QUESTION_WITH_TOP_NEWS_AND_OUTLINE.format(
                    article_title=request.title or "Chưa có tiêu đề",
    ...
    async def _generate_content_with_internal_reference(
        ...
        # ===== System prompt với outline structured =====
        system_prompt = self._generate_system_prompt(
            config=config,
            title=request.title,
            outline=request.outline,  # List[OutlineItem]
            main_keyword=request.main_keyword,
            secondary_keywords=request.secondary_keywords
        )
...
"""
        ...
        try:
            ...
            # Build SEO Article với generation_config
            seo_article = SEOArticle(
                title=seo_data.get("title", "").strip(),
                meta_description=seo_data.get("meta_description", "").strip(),
                blocks=blocks,
